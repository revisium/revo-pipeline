...
`@revisium/revo-pipeline` is an under-development prerelease ESM package with two
...
`package.json` exposes only `.` and `./kernel`. It builds strict NodeNext JavaScript and
...
workflow publishes from pull requests. Release validation and release-train transitions
are manual; automated npm publication requires an explicit version-matched alpha tag.
